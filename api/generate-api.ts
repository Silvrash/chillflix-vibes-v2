...
function getSchemaTypescriptType(text: string) {
  ...
  if (text.includes("@")) {
    ...
    if (text.includes("Object")) {
      return isOptional ? `Maybe<Record<string, any>${isArray ? "[]" : ""}>` : `Record<string, any>${isArray ? "[]" : ""}`;
    }

    if (text.includes("Date")) {
      return isOptional ? `Maybe<(Date | string)${isArray ? "[]" : ""}>` : `${isArray ? "(Date | string)[]" : "Date | string"}`;
    }
  }
